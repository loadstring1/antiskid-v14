return table.freeze{
	--meeeeeeee meeeeeeeeeeeeee creator of antiskid im always on top righttt
	1768834005, --Bakonowychlopak123 (main)
	3571934224, --Coza (alt)
	4711300696, --Android7User (alt)
	3570518464, --antiskidwashere (alt)
	
	--well trusted (or just my friends lol)
	3752146953, --groovy
	2346663041, --jalon
	2479604659, --egod
	6138298377, --vecko
	8143541330, --unlogged_able
	3632440858, --water enjoyer
	4206817123, --aqua4h
	7002708120, --noham911 - LOL THIS IS THE BIGGEST PLOT TWIST EVER
	137247575, --Zar0xz formerly known as supermario 
}